...
def index():
    if request.method == "GET":
        ...
        if not request.form.get("hour"):
            return render_template("apology.html", message="作業時間を入力してください", hint="作業時間は０時間～１０時間まで入力可能です")
        if not request.form.get("minute"):
            return render_template("apology.html",message="作業時間を入力してください", hint="作業時間は０分～５９分まで入力可能です")
        if not request.form.get("interval"):
            return render_template("apology.html",message="インターバルを入力してください", hint="インターバルは１分～５９分まで入力可能です")
        if not request.form.get("cycle"):
            return render_template("apology.html",message="サイクル（セット数）を入力してください", hint="サイクルは１回～２５回まで入力可能です")
        if not request.form.get("rest"):
            return render_template("apology.html",message="休憩時間を入力してください", hint="休憩時間は０分～１２０分までの範囲が入力可能です")
        if not request.form.get("rest_times"):
            return render_template("apology.html",message="休憩回数を入力してください", hint="休憩回数は０回～５回までの範囲が入力可能です")
        ...
        if hour == 0 and minute == 0:
            return render_template("apology.html", message="作業時間を入力してください", hint="作業時間の時間と分がどちらも０になっています")
